...
GroupingExpr::GroupingExpr(Token name) : Expr(EXPR_GROUPING) {
  this->name = name;
}
...
FunctionExpr::FunctionExpr(Expr* typeExpr, Token name, int arity, Expr** params, GroupingExpr* body, Expr* ui) : Expr(EXPR_FUNCTION) {
  this->typeExpr = typeExpr;
  this->name = name;
  this->arity = arity;
  this->params = params;
